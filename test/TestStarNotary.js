...
it("can add the star name and star symbol properly", async () => {
    let instance = await StarNotary.deployed();
    let starName = "Star Notary";
    let starSymbol = "SN";
    let user = accounts[1];
    // 1. create a Star with different tokenId
    let tokenId = 6;
    await instance.createStar("the bright star", tokenId, { from: user });
...
it("lets a user transfer a star", async () => {
    ...
    // 1. create a Star with different tokenId
    let tokenId = 9;
    await instance.createStar("the blue star", tokenId, { from: user1 });
...
it("lookUptokenIdToStarInfo test", async () => {
    ...
    // 1. create a Star with different tokenId
    let tokenId = 10;
    await instance.createStar("the shining star", tokenId, { from: user });
    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(tokenId);
    // 3. Verify if you Star name is the same
    assert.equal(starName, "the shining star");
});
